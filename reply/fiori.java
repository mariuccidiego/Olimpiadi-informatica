...
public class fiori {
    public int solve(int W, int H, int nFiori, int nRocce, int[][]campo) {
        ...
        for(int i=0;i<H;i++){
            for(int j=0;j<W;j++){
                if(campo[i][j]==0){
                    int distOra1=dist(0, 0, j, i);
                    System.out.println("distanza dalla 1="+distOra1);
                    int distOra2=dist(possFiori[1][1], possFiori[1][0], j, i);
                    System.out.println("distanza dalla 2="+distOra2);
                    System.out.println(possFiori[1][1]);
                    System.out.println("--------");
                    int dif = Math.abs(distOra2-distOra1);
                    if(minDif>dif){
                        minDif=dif;
                        possFiori[2][0]=i;
                        possFiori[2][1]=j;
